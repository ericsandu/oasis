...
async def run_cross_community():
    profile_path = "./data/synthetic_bridge_forced.json"
    generate_synthetic_profiles(profile_path)
    db_path = os.path.abspath("./data/bridge_forced.db")
    ...
    for step in range(1, 15):
        ...
        for idx, u in enumerate(sports_agents):
            ...
            # SCENARIO: PERFECT VIRALITY (Override)
            # If the tech post is in their feed, they WILL like it.
            if int(target_post_id) in feed:
                actions[u] = ManualAction(ActionType.LIKE_POST, {"post_id": target_post_id})
    ...
    plt.plot(history_steps, history_sports_reach, marker='o', color='green', linewidth=2)
    plt.title('Forced Deterministic Spillover (100% Infection Rate)')
    plt.xlabel('Timestep')
    plt.ylabel('Unique Impressions in Disjunct Crowd')
    plt.ylim(0, NUM_SPORTS)
    plt.axhline(y=0, color='gray', linestyle='--')
    plt.tight_layout()
    plt.savefig("scratch/forced_contagion_dashboard.png", dpi=300)
